...
git.Repository.prototype.merge = function(branch, flags, callback) {
  ...
  var gitMerge = new git.Command(this.path, 'merge', flags, branch)
    , repo = this;
 ...
 */
git.Repository.prototype.changeAuthor = function(author, complete) {
  var repo = this;
  var flags = ['--amend', '--author='+author ];

  var cmd = new git.Command(repo.path, 'commit', flags, '');
  cmd.exec(function(error, stdout, stderr) {
    var err = error || stderr;
